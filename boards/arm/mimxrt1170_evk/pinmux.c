...
static int mimxrt1170_evk_init(const struct device *dev)
{
	...
	imxrt_usdhc_dat3_cb_register(imxrt1170_evk_dat3_pull);
		...
		IOMUXC_SetPinMux(IOMUXC_GPIO_AD_18_SAI1_RX_SYNC,   0U);
		IOMUXC_SetPinMux(IOMUXC_GPIO_AD_19_SAI1_RX_BCLK,   0U);
		IOMUXC_SetPinMux(IOMUXC_GPIO_AD_20_SAI1_RX_DATA00, 0U);
	#else
...
#if DT_NODE_HAS_STATUS(DT_NODELABEL(sai4), okay) && CONFIG_I2S_MCUX_SAI
	#if DT_NODE_HAS_STATUS(DT_NODELABEL(enet), okay) \
	&& CONFIG_NET_L2_ETHERNET
		#error SAI4_TX_BCLK shared with ENET_RST on MIMXRT1170-EVK
	#endif

	CLOCK_EnableClock(kCLOCK_Iomuxc_Lpsr);

	IOMUXC_SetPinMux(IOMUXC_GPIO_LPSR_09_SAI4_TX_DATA, 7U);
	IOMUXC_SetPinMux(IOMUXC_GPIO_LPSR_10_SAI4_TX_SYNC, 7U);
	IOMUXC_SetPinMux(IOMUXC_GPIO_LPSR_12_SAI4_TX_BCLK, 7U);
#endif
	return 0;
}
